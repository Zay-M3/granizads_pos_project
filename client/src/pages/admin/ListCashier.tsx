...
import ModalEditUsers from "@components/cashiers/ModalEditUsers";
...
const ListCashier = () => {
  ...
  const [cajeroToDelete, setCajeroToDelete] = useState<UsuarioFormData | null>(
    null
  );
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [cajeroToEdit, setCajeroToEdit] = useState<UsuarioFormData | null>(
    null
  );

  // Cargar cajeros desde la API
  useEffect(() => {
  ...
  const handleEditClick = (cajero: UsuarioFormData) => {
    setCajeroToEdit(cajero);
    setIsEditModalOpen(true);
  };

  // Guardar cambios del usuario editado
  const handleSaveEdit = (updatedUser: UsuarioFormData) => {
    setCajeros((prevCajeros) =>
      prevCajeros.map((c) =>
        c.id_usuario === updatedUser.id_usuario ? updatedUser : c
      )
    );
  };
  ...
  const handleConfirmDelete = async () => {
    if (cajeroToDelete && cajeroToDelete.id_usuario) {
      try {
        await deleteUsuario(cajeroToDelete.id_usuario);
        setCajeros((prevCajeros) =>
            ...
            <tbody className="divide-y divide-secondary">
              {filteredCajeros.map((cajero) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div>
                      <p className="font-medium text-gray-800">
                        {cajero.fecha_nacimiento
                          ? calcularEdad(cajero.fecha_nacimiento)
                          : 0}{" "}
                        años
                      </p>
                  ...
                  <td className="px-6 py-4">{getRolBadge(cajero.rol)}</td>
                  <td className="px-6 py-4">
                    <div className="flex space-x-2">
                      <button
                        onClick={() => handleEditClick(cajero)}
