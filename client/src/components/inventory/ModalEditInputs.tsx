...
const ModalEditInputs = ({
  isOpen,
  onClose,
  insumo,
  onSave,
}: ModalEditInputsProps) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const getStockStatus = () => {
    if (stock <= minimo_stock * 0.5)
      return { color: "text-red-600", label: "Crítico", bgColor: "bg-red-100" };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          <div
            className={`${
              status.bgColor
            } rounded-xl p-5 border-2 border-dashed ${status.color.replace(
              "text-",
              "border-"
            )}`}
          >
            ...
            <div className="bg-white rounded-lg p-4 space-y-3">
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-sm font-bold ${status.bgColor} ${status.color}`}
                >
                  {status.label}
                </span>
              </div>
              ...
              <div className="pt-3 border-t border-gray-200">
                <div className="flex justify-between text-xs text-gray-600 mb-1">
                  ...
                  <span>
                    {(
                      (watch("stock") / (watch("minimo_stock") || 1)) *
                      100
                    ).toFixed(0)}
                    %
                  </span>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all ${
                      watch("stock") <= watch("minimo_stock") * 0.5
                        ? "bg-red-500"
                        : watch("stock") <= watch("minimo_stock")
                        ? "bg-orange-500"
                        : "bg-green-500"
                    }`}
                    style={{
                      width: `${Math.min(
                        (watch("stock") / (watch("minimo_stock") || 1)) * 100,
                        100
                      )}%`,
                    }}
                  />
